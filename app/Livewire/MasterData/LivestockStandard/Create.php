...
class Create extends Component
{
    ...
    public function mount($strainStandardId = null)
    {
        ...
        $this->livestockStrain = LivestockStrain::where('status', 'active')->get();
    ...
    {
        $livestockStrain = LivestockStrain::all();
        return view('livewire.master-data.livestock-standard.create', compact('livestockStrain'));
    }
    ...
    public function deleteStrainStandard($strainStandardId)
    {
        $strainStandard = LivestockStrainStandard::findOrFail($strainStandardId);

        // Check if there are associated livestock
        ...
        if ($strainStandard->standar_data) {
            // Dispatch a confirmation alert to the user
            $this->dispatch('confirm', [
                'message' => 'This Strain Standard has associated standard data. Do you really want to delete it?',
                'callback' => 'confirmDeleteStrainStandard',
                'id' => $strainStandardId
            ]);
        } else {
            // Proceed with deletion if no standard_data
            $strainStandard->delete();
            $this->dispatch('success', 'Strain Standard deleted successfully');
        }
    }

    public function confirmDeleteStrainStandard($strainStandardId)
    {
        $strainStandard = LivestockStrainStandard::findOrFail($strainStandardId);
        $strainStandard->delete();
        $this->dispatch('success', 'Strain Standard deleted successfully');
