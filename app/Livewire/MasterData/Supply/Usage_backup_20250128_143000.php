...
class Usage extends Component
{
    ...
    public function cancelSupplyUsage($id)
    {
        ...
        $usage->status = SupplyUsage::STATUS_CANCELLED;
        $usage->save();
    }
    ...
    private function createUsageWithService($service)
    {
        ...
        $payload = $this->buildSupplyUsagePayload();
        $result = $service->processWithMetadata($usage, $payload);
        logDebugIfDebug('Usage@createUsageWithService: Service processWithMetadata result.', $result);
        // SYNC CURRENT SUPPLY
        foreach ($this->items as $item) {
            if (!empty($item['supply_id'])) {
                $this->syncCurrentSupply($this->farm_id, $this->livestock_id, $item['supply_id']);
            }
            // --- Tambahan: Recalculate quantity_used pada stock terkait ---
            $stock = \App\Models\SupplyStock::find($item['supply_stock_id']);
            if ($stock) {
                $stock->recalculateQuantityUsed();
            }
        }
        // TODO: Audit trail, notification, etc.
        return $result;
    }
    ...
    private function updateUsageWithService($service)
    {
        ...
        if ($this->withHistory) {
            ...
            $payload = $this->buildSupplyUsagePayload();
            $result = $service->processWithMetadata($usage, $payload);
            logDebugIfDebug('Usage@updateUsageWithService: Service processWithMetadata result.', $result);
        } else {
            ...
            $usage->update(['total_quantity' => collect($this->items)->sum('converted_quantity')]);
            $result = ['success' => true, 'mode' => 'in-place-update'];
        }
        // SYNC CURRENT SUPPLY
        foreach ($this->items as $item) {
            if (!empty($item['supply_id'])) {
    ...
    public function deleteSupplyUsage($id)
    {
        DB::transaction(function () use ($id) {
            ...
            foreach ($supplyIds as $supplyId) {
                $this->syncCurrentSupply($this->farm_id, $this->livestock_id, $supplyId);
            }
        });
