...
class Usage extends Component
{
    ...
    public function updatedItems($value, $key)
    {
        ...
        if ($field === 'unit_id' && $value) {
            ...
            // Re-validate stock with new unit conversion
            if (!empty($this->items[$index]['quantity_taken'])) {
                $this->validateItemStock($index);
            }
        }
    ...
    private function validateStockAvailability()
    {
        ...
        foreach ($this->items as $index => $item) {
            ...
            $requestedQuantity = floatval($item['converted_quantity']); // GANTI: gunakan converted_quantity
            ...
            if ($requestedQuantity > $availableStock) {
                ...
                $validationErrors["items.{$index}.converted_quantity"] =
                    "Quantity untuk {$supplyName} melebihi stock yang tersedia! " .
                    "Stock tersedia: " . number_format($availableStock, 2) . ", " .
                    "Diminta: " . number_format($requestedQuantity, 2) . ", " .
                    "Kekurangan: " . number_format($shortage, 2);
    ...
    public function save()
    {
        ...
        try {
            // First validate basic form rules
            $this->validate();
            logDebugIfDebug('Usage@save: Basic validation passed.');

            // Then validate stock availability with enhanced checking
            ...
            // Dispatch user-friendly error message
    ...
    private function convertToSmallestUnit($item)
    {
        ...
        return ($item['quantity_taken'] * $selectedUnit['value']) / $smallestUnit['value'];
    }

    /**
     * Create new supply usage record with enhanced validation
     */
    private function createUsage()
    {
        $companyId = Auth::user()->company_id;
        $total_quantity = collect($this->items)->sum('converted_quantity');

        logDebugIfDebug('Usage@createUsage: Creating new usage record.', [
            'company_id'   => $companyId,
            'farm_id'      => $this->farm_id,
            'coop_id'      => $this->coop_id,
        ...
        logDebugIfDebug('Usage@createUsage: Usage record created.', ['usage_id' => $usage->id]);

        logDebugIfDebug('Usage@createUsage: Creating usage details.', ['usage_id' => $usage->id, 'items_count' => count($this->items)]);
        $this->createUsageDetails($usage);
        logDebugIfDebug('Usage@createUsage: Usage details created.', ['usage_id' => $usage->id]);

        logDebugIfDebug('Usage@createUsage: Updating supply stocks.', ['usage_id' => $usage->id]);
        $this->updateSupplyStocks($usage);
        logDebugIfDebug('Usage@createUsage: Supply stocks updated.', ['usage_id' => $usage->id]);

        logDebugIfDebug('Usage@createUsage: Process completed for usage.', ['usage_id' => $usage->id]);
    }

    private function updateUsage()
    {
        $usage = SupplyUsage::findOrFail($this->usageId);
        logDebugIfDebug('Usage@updateUsage: Starting update.', ['usage_id' => $usage->id]);
        ...
        logDebugIfDebug('Usage@updateUsage: Usage record updated.');

        $usage->details()->delete();
        logDebugIfDebug('Usage@updateUsage: Old details deleted.');

        $this->createUsageDetails($usage);
        $this->updateSupplyStocks($usage);
        logDebugIfDebug('Usage@updateUsage: Process completed for usage.', ['usage_id' => $usage->id]);
    }
    ...
    public function deleteSupplyUsage($id)
    {
        ...
        // session()->flash('message', 'Supply usage berhasil dihapus.');
