...
use App\Services\Recording\DepletionDataStandardizationService;
...
class Records extends Component
{
    ...
    protected ?DepletionDataStandardizationService $standardizationService = null;
    ...
    public function mount(
        ...
        DepletionDataStandardizationService $standardizationService
    ) {
    ...
    private function loadLivestockConfiguration(Livestock $livestock): void
    {
        ...
        Log::info('Records - Livestock Configuration Loaded', [
            ...
            'depletion_method' => $this->isManualDepletionEnabled ? 'manual' : 'recording'
        ]);
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($this->mortality > 0) {
                ...
                $mortalityResult = $this->storeDeplesiWithDetails(LivestockDepletionConfig::TYPE_MORTALITY, $this->mortality, $recording->id);
            ...
            if ($this->culling > 0) {
                ...
                $cullingResult = $this->storeDeplesiWithDetails(LivestockDepletionConfig::TYPE_CULLING, $this->culling, $recording->id);
    ...
    private function storeDeplesiWithDetails($jenis, $jumlah, $recordingId)
    {
        ...
        if ($this->shouldUseFifoDepletion($livestock, $jenis)) {
            ...
            if ($fifoResult && (is_array($fifoResult) ? ($fifoResult['success'] ?? false) : true)) {
                // Standardize FIFO depletion records to match traditional format using service
                if ($this->standardizationService) {
                    $standardizationResult = $this->standardizationService->standardizeFifoDepletionRecords(
                        $fifoResult,
                        $livestock,
                        $jenis,
                        $recordingId,
                    ...
                    Log::info('📝 Records: FIFO standardization completed', [
                        'livestock_id' => $livestock->id,
                        'standardization_success' => $standardizationResult['success'] ?? false,
                        'processed_records' => $standardizationResult['processed_records'] ?? 0
                    ]);
                }
                return $fifoResult;
            }
        ...
        // Get batch information for traditional depletion
        $batchInfo = $this->getBatchInfoForTraditionalDepletion($livestock, $jumlah);
        ...
        $deplesi = LivestockDepletion::updateOrCreate(
            ...
            [
                ...
                'metadata' => [
                    ...
                    // Batch information for traditional method
                    'batch_information' => $batchInfo
                ],
                'data' => [
                    'depletion_method' => 'traditional',
                    'original_request' => $jumlah,
                    'processing_source' => 'Records Component',
                    'batch_processing' => $batchInfo['batch_count'] > 1,
                    'single_record' => true,

                    // Enhanced batch information
                    'batch_details' => $batchInfo['batch_details'],
                    'batch_distribution' => $batchInfo['batch_distribution'],
                    'primary_batch_id' => $batchInfo['primary_batch']['id'] ?? null,
                    'primary_batch_name' => $batchInfo['primary_batch']['name'] ?? null,
                    'affected_batches_count' => $batchInfo['batch_count'],

                    // Calculation details
                    'calculation_method' => $batchInfo['calculation_method'],
                    'distribution_strategy' => $batchInfo['distribution_strategy'],
                    'total_available_before' => $batchInfo['total_available_before'],
                    'total_available_after' => $batchInfo['total_available_after'],
        ...
        Log::info("📝 Recorded livestock depletion with batch information", [
            'livestock_id' => $this->livestockId,
            'date' => $this->date,
            'type' => $jenis,
            'quantity' => $jumlah,
            'recording_id' => $recordingId,
            'method' => 'traditional',
            'batch_count' => $batchInfo['batch_count'],
            'primary_batch_id' => $batchInfo['primary_batch']['id'] ?? null
        ]);
    ...
    /**
     * Get batch information for traditional depletion method
     * 
     * @param Livestock $livestock
     * @param int $depletionQuantity
     * @return array
     */
    private function getBatchInfoForTraditionalDepletion(Livestock $livestock, int $depletionQuantity): array
    {
        // Get all active batches for this livestock
        $batches = $livestock->batches()
            ->where('status', 'active')
            ->whereRaw('(initial_quantity - quantity_depletion - quantity_sales - quantity_mutated) > 0')
            ->orderBy('start_date', 'asc') // Use FIFO ordering for consistency
            ->get();
        ...
        if ($batches->isEmpty()) {
            return [
                ...
                'batch_distribution' => [],
                'primary_batch' => null,
                'calculation_method' => 'no_batches',
                'distribution_strategy' => 'not_applicable',
        ...
        $remainingDepletion = $depletionQuantity;
        $primaryBatch = null;

        foreach ($batches as $batch) {
            $availableInBatch = $batch->initial_quantity - $batch->quantity_depletion - $batch->quantity_sales - $batch->quantity_mutated;
            $totalAvailableBefore += $availableInBatch;

            // For traditional method, we simulate how depletion would be distributed
            $depletionFromThisBatch = min($remainingDepletion, $availableInBatch);

            if ($depletionFromThisBatch > 0) {
                if (!$primaryBatch) {
                    $primaryBatch = $batch; // First batch with depletion becomes primary
                }

                $batchDistribution[] = [
                    'batch_id' => $batch->id,
                    'batch_name' => $batch->name,
                    'depletion_quantity' => $depletionFromThisBatch,
                    'available_before' => $availableInBatch,
                    'available_after' => $availableInBatch - $depletionFromThisBatch,
                    'percentage_of_total' => $depletionQuantity > 0 ? round(($depletionFromThisBatch / $depletionQuantity) * 100, 2) : 0
                ];

                $remainingDepletion -= $depletionFromThisBatch;
            }
            ...
            $batchDetails[] = [
                ...
                'current_depletion' => $batch->quantity_depletion,
                'current_sales' => $batch->quantity_sales,
                'current_mutated' => $batch->quantity_mutated,
        ...
        $totalAvailableAfter = $totalAvailableBefore - $depletionQuantity;
        ...
        return [
            ...
            'primary_batch' => $primaryBatch ? [
                ...
                'age_days' => now()->diffInDays($primaryBatch->start_date)
            ] : null,
            'calculation_method' => 'traditional_with_batch_simulation',
            'distribution_strategy' => $batches->count() > 1 ? 'fifo_simulation' : 'single_batch',
            'total_available_before' => $totalAvailableBefore,
            'total_available_after' => $totalAvailableAfter,
            'depletion_coverage' => $depletionQuantity <= $totalAvailableBefore ? 'full' : 'partial',
            'shortage' => max(0, $depletionQuantity - $totalAvailableBefore)
        ];
    }

    /**
     * Check if FIFO depletion should be used for this livestock and depletion type
     * Uses the validation service for consistent checking
     *
     * @param Livestock $livestock
     * @param string $jenis
     * @return bool
    ...
    {
        try {
            Log::info('🔍 FIFO Check: Starting FIFO depletion validation', [
                'livestock_id' => $livestock->id,
                'depletion_type' => $jenis,
                'validation_service_available' => $this->validationService ? 'yes' : 'no',
                'fifo_service_available' => $this->fifoDepletionService ? 'yes' : 'no'
            ]);

            // Get current configuration
            $config = $livestock->getConfiguration();
            $currentMethod = $config['depletion_method'] ?? 'manual';

            Log::info('🔍 FIFO Check: Current configuration', [
                'livestock_id' => $livestock->id,
                'current_method' => $currentMethod,
                'full_config' => $config
            ]);

            // If method is not FIFO, return false immediately
            ...
            // Use validation service for comprehensive FIFO requirement checking
            if ($this->validationService) {
                $fifoValidation = $this->validationService->validateFifoRequirements($livestock, 'depletion');

                Log::info('🔍 FIFO Check: Validation service result', [
                    'livestock_id' => $livestock->id,
                    'depletion_type' => $jenis,
                    'validation_result' => $fifoValidation
                ]);

                if (!$fifoValidation['valid']) {
                    Log::warning('⚠️ FIFO Check: FIFO requirements not met, falling back to traditional', [
                        'livestock_id' => $livestock->id,
                        'reason' => $fifoValidation['message'] ?? 'Unknown validation failure'
                    ]);
                    return false;
                }
            } else {
                Log::warning('⚠️ FIFO Check: Validation service not available, using basic checks', [
                    'livestock_id' => $livestock->id
                ]);
            }
            ...
            if (!$this->fifoDepletionService) {
                Log::warning('⚠️ FIFO Check: FIFODepletionService not available', [
                    'livestock_id' => $livestock->id
                ]);
                return false;
            }

            Log::info('✅ FIFO Check: All requirements met, using FIFO depletion', [
                'livestock_id' => $livestock->id,
                'depletion_type' => $jenis,
