...
class FeedUsageDetailObserver
{
    public function saved($detail)
    {
        if (method_exists($detail, 'feedUsage') && $detail->feedUsage) {
            $livestockId = $detail->feedUsage->livestock_id;
            $tanggal = $detail->feedUsage->usage_date;
            if ($livestockId && $tanggal) {
                ...
                if ($cost) {
                    $cost->markInvalidCalculation('Feed usage diubah pada hari ini', $cost->toArray());
                }
