...
class ternakJualDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->rawColumns(['farm_id'])
            ->editColumn('total_berat', function (TernakJual $ternak) {
            ...
            ->editColumn('tanggal', function (TernakJual $ternak) {
                return $ternak->tanggal->format('d M Y, h:i a');
            })
            ->editColumn('quantity', function (TernakJual $ternak) {
                return $ternak->quantity . ' Ekor';
            })
            ->editColumn('umur', function (TernakJual $ternak) {
                return $ternak->umur . ' Hari';
            })
            ->editColumn('id', function (TernakJual $ternak) {
                return strtoupper(substr(strrchr($ternak->id, '-'), 4));
            })
            ->editColumn('kelompok_ternak_id', function (TernakJual $ternak) {
                return $ternak->kelompokTernaks->name;
            })
            ->editColumn('farm_id', function (TernakJual $ternak) {
                return $ternak->kelompokTernaks->transaksiBeli->farms->nama;
            })
            ->editColumn('kandang_id', function (TernakJual $ternak) {
                return $ternak->kelompokTernaks->transaksiBeli->kandangs->nama;
            })
            ->editColumn('created_at', function (TernakJual $ternak) {
     ...
     */
    public function query(TernakJual $model): QueryBuilder
    {
        if (auth()->user()->hasRole('Operator')) {
    ...
    public function html(): HtmlBuilder
{
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('umur')->title('Umur Ternak'),
            Column::make('status'),
    ...
    {
        return 'TernakJuals_' . date('YmdHis');
    }
}
