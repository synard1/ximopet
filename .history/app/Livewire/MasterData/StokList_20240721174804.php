...
class StokList extends Component
{
    ...
    {
        $this->validate();
        Kandang::updateOrCreate(['id' => $this->stok_id], [
            'kode' => $this->kode,
            'nama' => $this->nama,
        ...
        // $this->stok_id ? 'Kandang updated successfully.' : 'Kandang created successfully.');
        $this->closeModal();
        $this->resetInputFields();
    }
    ...
    {
        $stok = Kandang::where('id',$id)->first();
        $this->stok_id = $id;
        $this->kode = $stok->kode;
        $this->nama = $stok->nama;
