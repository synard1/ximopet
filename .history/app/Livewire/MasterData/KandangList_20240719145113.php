...
class KandangList extends Component
{
    ...
    {
        $this->validate();
        Kandang::updateOrCreate(['id' => $this->farm_id], [
            'jenis' => "KandangList",
            'kode' => $this->kode,
            'nama' => $this->nama,
            'alamat' => $this->alamat,
        ...
        // $this->farm_id ? 'Kandang updated successfully.' : 'Kandang created successfully.');
        $this->closeModal();
        $this->resetInputFields();
    }
    ...
    {
        $farm = Kandang::where('id',$id)->first();
        $this->farm_id = $id;
        $this->kode = $farm->kode;
        $this->nama = $farm->nama;
