...
{
    public $isOpen = 0;
    public $jenis, $nama_farm, $nama_operator, $status;

    protected $rules = [
        'jenis' => 'required', // Unique except on update
        'nama_farm' => 'required',
    ...
    public function render()
    {
        ...
        // Load operators associated with the selected farm (if any)
        if (!empty($this->selectedFarm)) {
            $this->operators = FarmOperator::where('farm_id', $this->selectedFarm)
                ->pluck('nama_operator', 'id'); 
        } else {
            $this->operators = [];
        }
        return view('livewire.master-data.tambah-operator-farm', ['availableFarms' => $availableFarms]);
    }
