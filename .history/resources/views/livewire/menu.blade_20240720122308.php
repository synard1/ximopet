		...
		var fetchFarm = function (e) {
			...
			fetch(apiUrl)
				...
				.then(data => {
					...
					const defaultOption = new Option("=== Pilih Farm ===", "", true, true); // Empty value, selected, disabled
            farmSelect.append(defaultOption);
			
					data.forEach(farm => {
						const option = new Option(farm.nama, farm.nama); // Use nama for both text and value
						option.setAttribute('data-farm-id', farm.id); // Store the original ID as data attribute (optional)
