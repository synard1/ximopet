...
class Records extends Component
{
            ...
            if ($this->skipConfigMultipleBatch) {
                ...
                if ($livestock->getActiveBatchesCount() > 1 && !\App\Models\Recording::where('livestock_id', $livestock->id)->exists()) {
                    ...
                    if (empty($config) || !is_array($config) || empty($config['recording_method'])) {
                        ...
                        // Log untuk debugging
                        logErrorIfDebug('[Records] setRecords: Gagal lanjut, config belum diatur untuk livestock_id: ' . $livestock->id, [
                            'config' => $config,
    ...
    public function updatedDate($value)
    {
        ...
        if ($deplesi->isNotEmpty()) {
            ...
            logInfoIfDebug('Current date depletion processed with config system', [
                'livestock_id' => $this->livestockId,
                'selected_date' => $value,
            ...
            logInfoIfDebug('No current date depletion data found', [
                'livestock_id' => $this->livestockId,
                'selected_date' => $value
            ]);
        }
    ...
    private function loadYesterdayData($yesterdayDate)
    {
        ...
        try {
            ...
            if ($yesterdayDeplesi->isNotEmpty()) {
                ...
                logInfoIfDebug('Yesterday depletion processed with config system', [
                    'livestock_id' => $this->livestockId,
                ...
                logInfoIfDebug('No yesterday depletion data found', [
                    'livestock_id' => $this->livestockId,
                    'yesterday_date' => $yesterdayDate
                ]);
            }
            ...
            logInfoIfDebug("Yesterday data loaded successfully", [
                'livestock_id' => $this->livestockId,
                'yesterday_date' => $yesterdayDate,
            ...
            logErrorIfDebug("Error loading yesterday data: " . $e->getMessage(), [
                'livestock_id' => $this->livestockId,
                'yesterday_date' => $yesterdayDate,
                'error' => $e->getMessage()
            ]);
    ...
    public function save()
    {
        ...
        logInfoIfDebug('🚀 Records Save: Method called', [
            'livestock_id' => $this->livestockId,
        ...
        logInfoIfDebug('🔍 Records Save: Starting validation');

        try {
            $this->validate();
            logInfoIfDebug('✅ Records Save: Validation passed');
        } catch (ValidationException $e) {
            logErrorIfDebug('❌ Records Save: Validation failed', [
        ...
        try {
            ...
            logInfoIfDebug('🧪 Records Save: Preparing supply usage data');
            ...
            logInfoIfDebug('🐄 Records Save: Validating livestock data');
            ...
            logInfoIfDebug('📈 Records Save: Getting population history');
            ...
            logInfoIfDebug('🔢 Records Save: Validating outflows');
            ...
            logInfoIfDebug('⏰ Records Save: Calculating age and stock values');
            ...
            logInfoIfDebug('📦 Records Save: Preparing detailed payload');
            ...
            if (!empty($this->supplyUsages) || $this->hasSupplyChanged === true) {
                try {
                    ...
                    logInfoIfDebug("Supply usage processed successfully", [
            ...
            logInfoIfDebug('💀 Records Save: Processing depletion data', [
                'mortality' => $this->mortality,
                'culling' => $this->culling,
            ...
            foreach ($depletionsToProcess as $depletionType => $quantity) {
                if ($quantity > 0) {
                    ...
                    logInfoIfDebug("Records Save: Processing depletion.", [
                        'type'         => $depletionType,
                        'quantity'     => $quantity,
                        'recording_id' => $recording->id,
                    ]);
                    ...
                    logInfoIfDebug("Records Save: Depletion for type '{$depletionType}' processed.", [
                    ...
                    if (!$isSuccess) {
                        ...
                        logErrorIfDebug("Failed to process depletion for type '{$depletionType}'", [
                            'livestock_id' => $this->livestockId,
                            'recording_id' => $recording->id,
                            'response'     => $result,
                        ]);
            ...
            logInfoIfDebug('💾 Records Save: Committing database transaction');
            ...
            logInfoIfDebug('🎉 Records Save: Process completed successfully');
            ...
            logErrorIfDebug("[$class::$method] Error: $message | Line: $line | File: $file");
            logDebugIfDebug("[$class::$method] Stack trace: " . $e->getTraceAsString());
    ...
    private function saveFeedUsageWithTracking($data, $recordingId)
    {
        if ($this->feedUsageId) {
            ...
            logInfoIfDebug("Reverting {$oldDetails->count()} feed usage details for usage ID {$usage->id}");
            ...
            foreach ($oldDetails as $detail) {
                ...
                if ($stock) {
                    ...
                    logInfoIfDebug("Reverting feed stock usage", [
                        'stock_id' => $stock->id,
            ...
            foreach ($currentSupplyChanges as $feedId => $quantity) {
                ...
                if ($currentSupply) {
                    ...
                    logInfoIfDebug("Updated CurrentSupply after reversion", [
                        'livestock_id' => $this->livestockId,
                        'feed_id' => $feedId,
        ...
        foreach ($this->usages as $usageData) {
            ...
            if ($currentSupply) {
                ...
                logInfoIfDebug("Updated CurrentSupply for new usage", [
                    'livestock_id' => $this->livestockId,
        ...
        logInfoIfDebug("Feed usage processed", [
            'usage_id' => $usage->id,
    ...
    private function saveSupplyUsageWithTracking($data, $recordingId)
    {
        if ($this->supplyUsageId) {
            ...
            logInfoIfDebug("Reverting {$oldDetails->count()} supply usage details for usage ID {$usage->id}");
            ...
            foreach ($oldDetails as $detail) {
                ...
                if ($stock) {
                    // Store reversion details for audit trail
                    logInfoIfDebug("Reverting supply stock usage", [
                        'stock_id' => $stock->id,
            ...
            foreach ($currentSupplyChanges as $supplyId => $quantity) {
                ...
                if ($currentSupply) {
                    ...
                    logInfoIfDebug("Updated CurrentSupply after supply reversion", [
                        'livestock_id' => $this->livestockId,
                        'supply_id' => $supplyId,
                        'old_quantity' => $oldQuantity,
                        'added_quantity' => $quantity,
        ...
        logInfoIfDebug("Supply usage processed", [
            'usage_id' => $usage->id,
            'livestock_id' => $usage->livestock_id,
            'date' => $usage->usage_date,
            'total_quantity' => $usage->total_quantity,
    ...
    private function processSupplyUsageDetail($usage, $usageData)
    {
        ...
        foreach ($availableStocks as $stock) {
            ...
            if ($quantityToTake > 0) {
                ...
                logInfoIfDebug("Supply usage detail created", [
                    'usage_id' => $usage->id,
                    'supply_id' => $usageData['supply_id'],
                    'stock_id' => $stock->id,
                    'quantity_taken' => $quantityToTake,
        ...
        if ($quantityNeeded > 0) {
            logErrorIfDebug("Insufficient stock for supply usage", [
                'supply_id' => $usageData['supply_id'],
    ...
    private function storeDeplesiWithDetails($jenis, $jumlah, $recordingId)
    {
        ...
        logInfoIfDebug('📝 StoreDeplesi: Method called with config normalization', [
            'original_type' => $jenis,
        ...
        if ($jumlah <= 0) {
            logInfoIfDebug('⚠️ StoreDeplesi: Zero quantity, skipping');
            return null;
        }

        $livestock = Livestock::find($this->livestockId);
        if (!$livestock) {
            logErrorIfDebug('❌ StoreDeplesi: Livestock not found', ['livestock_id' => $this->livestockId]);
            return null;
        }

        logInfoIfDebug('✅ StoreDeplesi: Livestock found', [
        ...
        logInfoIfDebug("📝 Recorded livestock depletion", [
            'livestock_id' => $this->livestockId,
            'date' => $this->date,
            'type' => $jenis,
            'quantity' => $jumlah,
    ...
    //         logErrorIfDebug('🔄 FIFO depletion failed, falling back to traditional method', [
    //             'livestock_id' => $livestock->id,
    ...
    private function standardizeFifoDepletionRecords(array $fifoResult, Livestock $livestock, string $jenis, string $recordingId, int $age): void
    {
        try {
            ...
            foreach ($depletionRecords as $recordData) {
                ...
                logInfoIfDebug('📝 Standardized FIFO depletion record', [
            ...
            logInfoIfDebug('✅ FIFO depletion records standardized successfully', [
                'livestock_id' => $livestock->id,
                'depletion_type' => $jenis,
            ...
            logErrorIfDebug('❌ Failed to standardize FIFO depletion records', [
                'livestock_id' => $livestock->id,
                'depletion_type' => $jenis,
                'error' => $e->getMessage(),
    ...
    private function updateCurrentLivestockQuantityWithHistory()
    {
        ...
        if (!$livestock || !$currentLivestock) {
            logErrorIfDebug('⚠️ Livestock or CurrentLivestock not found', [
                'livestock_id' => $this->livestockId,
