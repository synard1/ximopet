...
class SupplyMutationDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('quantity', function (Mutation $transaction) {
                $total = 0;
                $unitName = '';
                if ($transaction->supplyMutation && $transaction->supplyMutation->supplyMutationItems && $transaction->supplyMutation->supplyMutationItems->count() > 0) {
                    $items = $transaction->supplyMutation->supplyMutationItems;
                    ...
                    // Jika unit berbeda, $unitName tetap kosong
                }
    ...
    public function query(Mutation $model): QueryBuilder
    {
        ...
        $query = $model->where('type', 'supply')->with(['supplyMutation.supplyMutationItems.convertedUnit'])->newQuery();
