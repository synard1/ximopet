        ...
        element.addEventListener("click", function () {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch("delete_stok", [
                        this.getAttribute("data-kt-stok-id"),
                    ]);
                }
            });
        });
...
// Add click event listener to update buttons
document
    .querySelectorAll('[data-kt-action="view_detail_supplystocks"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === "success") {
                ...
                $("#feedstockDetailsContainer").html(
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === "success") {
                ...
                $("#supplystockDetailsContainer").html(
                    '<p class="text-danger">Gagal memuat data.</p>'
                );
            }
        },
            ...
            <div class="accordion-item mb-4 border border-primary rounded">
                <h2 class="accordion-header">
                    <button class="accordion-button fw-bold collapsed" type="button" data-bs-toggle="collapse"
                        ...
                        ${feed.feed_name} - Batch ${feed.no_batch} | Tanggal: ${
            feed.tanggal
        } | Harga: Rp ${parseFloat(feed.harga).toLocaleString("id-ID")}
                    </button>
                </h2>
        ...
        const card = `
            <div class="accordion-item mb-4 border border-primary rounded">
                <h2 class="accordion-header">
                    <button class="accordion-button fw-bold collapsed" type="button" data-bs-toggle="collapse"
                        data-bs-target="#${batchId}" aria-expanded="false" aria-controls="${batchId}">
                        ${supply.supply_name} - Batch ${
            supply.no_batch
        } | Tanggal: ${supply.tanggal} | Harga: Rp ${parseFloat(
            supply.harga
...
function getFeedAkumulasiHistorikal(livestockId, feedId, startDate, endDate) {
    $.ajax({
        url: "/api/v2/feed/usages/akumulasi",
        type: "POST",
        data: {
            livestock_id: livestockId,
            feed_id: feedId,
        ...
        success: function (response) {
            if (response.status === "success") {
                renderFeedAkumulasiHistorikalTable(response.data);
            } else {
                $("#feedstockDetailsContainer").html(
                    '<p class="text-danger">Gagal memuat data akumulasi historikal.</p>'
                );
            }
        },
        error: function (xhr) {
...
function renderFeedAkumulasiHistorikalTable(data) {
    const container = $("#feedstockDetailsContainer");
    container.empty();

    let tableHtml = `
        <div class="mb-3 d-flex justify-content-between align-items-center">
            <h5 class="fw-bold mb-0">Akumulasi Historikal (All Stocks Combined)</h5>
            <div class="d-flex gap-2">
                <button class="btn btn-outline-success btn-sm" id="exportAkumulasiExcel">Export Excel</button>
                <button class="btn btn-outline-dark btn-sm" id="printAkumulasiTable">Print</button>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-sm table-bordered" id="akumulasiHistorikalTable">
                <thead class="table-light">
                    <tr>
                        <th class="text-center">Tanggal</th>
                        <th class="text-start">Keterangan</th>
                ...
                <tbody>
                    ${data
                        ...
                        <tr>
                            ...
                            <td class="text-end fw-bold text-success">${parseFloat(
                                row.sisa
                            ).toLocaleString("id-ID")}</td>
    ...
    // Export Excel
    $("#exportAkumulasiExcel")
        .off("click")
        .on("click", function () {
            const table = document.getElementById("akumulasiHistorikalTable");
            if (!table) return;
            const tableHTML = table.outerHTML.replace(/ /g, "%20");
            const a = document.createElement("a");
            a.href = "data:application/vnd.ms-excel," + tableHTML;
            a.download = `feedstock_akumulasi_historikal.xls`;
            a.click();
        });
    // Print
    $("#printAkumulasiTable")
        ...
        .on("click", function () {
            ...
            printWindow.document.write(
                "<html><head><title>Print Akumulasi Historikal</title>"
            );
            printWindow.document.write(
                "<style>table { width: 100%; border-collapse: collapse; margin-bottom: 40px; } th, td { border: 1px solid #ccc; padding: 6px; }</style>"
            );
...
document
    .querySelectorAll('[data-kt-action="view_detail_feedstoks"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
            ...
            const livestockId =
                event.currentTarget.getAttribute("data-livestock-id");
...
// Add click event listener to update buttons
document
    .querySelectorAll('[data-kt-action="view_detail_stok"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
...
document
    .querySelectorAll('[data-kt-action="supply_transfer_row"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
...
document
    .querySelectorAll('[data-kt-action="transfer_row"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
            ...
            // Get transaksi ID
            const transaksiId =
                event.currentTarget.getAttribute("data-kt-stok-id");
...
document
    .querySelectorAll('[data-kt-action="feed_transfer_row"]')
    .forEach(function (element) {
        element.addEventListener("click", function (e) {
            e.preventDefault();
            // destroyDetailsTable();
