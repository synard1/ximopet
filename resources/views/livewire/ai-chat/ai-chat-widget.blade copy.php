<div x-data="{
    ...
    providerStatus: @entangle('providerStatus'),
    ...
    // Debug config values
    console.log('Chat UI Config:', {
        show_templates_button: {{ config('chat.ui.show_templates_button', true) ? 'true' : 'false' }},
        show_search_button: {{ config('chat.ui.show_search_button', true) ? 'true' : 'false' }},
        show_templates_button_raw: {{ json_encode(config('chat.ui.show_templates_button')) }},
        show_search_button_raw: {{ json_encode(config('chat.ui.show_search_button')) }},
        show_templates_button_strict: {{ config('chat.ui.show_templates_button') ? 'true' : 'false' }},
        show_search_button_strict: {{ config('chat.ui.show_search_button') ? 'true' : 'false' }}
    });
...
" class="ai-chat-widget" :class="{
    ...
    <div x-show="!isOpen" x-cloak x-transition:enter="transition ease-out duration-200"
        ...
        x-transition:leave-end="opacity-0 scale-90" class="chat-bubble-wrapper" style="position: relative;">
        <button @click="toggleChat()" class="btn btn-primary rounded-circle chat-toggle-btn"
            :class="{ 'show': !isOpen }"
            style="width: 48px; height: 48px; box-shadow: 0 4px 12px rgba(0, 123, 255, 0.3);" title="Open AI Chat">
    ...
    <div class="chat-window-container" x-show="isOpen" x-cloak
        ...
        <!-- Chat Window -->
        <div class="chat-window card" x-show="showChatWindow" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center p-3">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="btn-group me-2" role="group">
                        <button
                            @click="showSettings = !showSettings; showSessions = false; showTemplates = false; showSearch = false"
                            class="btn btn-sm btn-outline-light chat-header-btn settings-btn-{{ time() }}"
                            :class="{ 'active': showSettings }" title="Settings"
                        ...
                        <button
                            @click="showSessions = !showSessions; showSettings = false; showTemplates = false; showSearch = false"
                            class="btn btn-sm btn-outline-light chat-header-btn history-btn-{{ time() }}"
                            :class="{ 'active': showSessions }" title="Chat Sessions"
                            style="color: white !important; background-color: rgba(255,255,255,0.4) !important; border: 2px solid white !important; font-weight: bold !important; text-shadow: 0 1px 1px rgba(0,0,0,0.5) !important; padding: 0.25rem 0.75rem !important; box-shadow: 0 0 5px rgba(255,255,255,0.3) !important;"
                            x-show="{{ config('chat.ui.show_history_button', true) ? 'true' : 'false' }}">
                            <i class="fas fa-history"></i>
                        </button>

                        @if(config('chat.ui.show_templates_button', false))
                        <button
                            @click="showTemplates = !showTemplates; showSettings = false; showSessions = false; showSearch = false"
                        ...
                        @if(config('chat.ui.show_search_button', false))
                        <button
                            @click="showSearch = !showSearch; showSettings = false; showSessions = false; showTemplates = false"
                    ...
                    @if(config('chat.ui.show_minimize_button', false))
                    <!-- Minimize Button -->
                    <button wire:click="toggleMinimize"
                        class="btn btn-sm btn-outline-light chat-header-btn minimize-btn me-2"
                        ...
                        x-show="{{ config('chat.ui.show_minimize_button', true) ? 'true' : 'false' }}" title="Minimize">
                        <i class="fas" :class="$wire.isMinimized ? 'fa-window-maximize' : 'fa-window-minimize'"></i>
                    </button>
                    @endif

                    <!-- Close Button -->
                    <button @click="closeChat()" class="btn btn-sm btn-outline-light chat-header-btn close-btn"
                        style="color: white !important; background-color: rgba(255,255,255,0.4) !important; border: 1px solid rgba(255,255,255,0.7) !important; padding: 0.25rem 0.75rem !important;"
                        title="Close chat">
            ...
            <div x-show="showSettings" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button @click="showSettings = false" class="btn btn-sm btn-outline-secondary"
                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="btn-group" role="group">
                        <button type="button" class="btn btn-sm btn-outline-secondary dropdown-toggle"
                            data-bs-toggle="dropdown" aria-expanded="false" @disabled(!$currentSessionId)>
                            <i class="fas fa-download"></i> Export
                        </button>
                        <ul class="dropdown-menu">
                            @foreach($exportFormats as $format => $label)
                            <li>
                                <button class="dropdown-item" wire:click="exportChatAs('{{ $format }}')"
                                    wire:loading.attr="disabled">
                                    {{ $label }}
                                </button>
                            </li>
            ...
            <div x-show="showSessions" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="sessions-list-container" style="max-height: 300px; overflow-y: auto;">
                    ...
                    <div class="session-item mb-2 p-3 rounded {{ $session['id'] === $currentSessionId ? 'bg-primary text-white' : 'bg-white border' }} cursor-pointer"
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <button @click="if(confirm('Delete this session?')) { $wire.deleteSession('{{ $session['id'] }}') }"
                                class="btn btn-sm {{ $session['id'] === $currentSessionId ? 'btn-outline-light' : 'btn-outline-danger' }} ms-2"
                                wire:loading.attr="disabled" wire:target="deleteSession"
                                title="Delete session">
                                <i class="fas fa-trash fa-xs"></i>
                                <span wire:loading wire:target="deleteSession"
                ...
                <div class="mt-3 pt-3 border-top d-flex justify-content-between align-items-center">
                    ...
                    @if(config('chat.ui.enable_session_delete', true))
                    <button @click="if(confirm('Clear all chat sessions?')) { $wire.clearAllSessions() }" class="btn btn-sm btn-outline-warning"
                        wire:loading.attr="disabled" wire:target="clearAllSessions"
                        title="Clear all sessions">
                        <i class="fas fa-trash-alt fa-xs me-1"></i>Clear All
                        <span wire:loading wire:target="clearAllSessions" class="spinner-border spinner-border-sm ms-1"
                            role="status"></span>
                    </button>
                    @endif
                </div>
            ...
            <div x-show="showTemplates" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button @click="showTemplates = false" class="btn btn-sm btn-outline-secondary"
            ...
            <div x-show="showSearch" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button @click="showSearch = false" class="btn btn-sm btn-outline-secondary" title="Close Search">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <form wire:submit.prevent="searchChatHistory" class="mb-3">
                    <div class="input-group">
                        <input type="text" wire:model.defer="searchQuery" class="form-control form-control-sm"
                            placeholder="Search sessions and messages..." aria-label="Search chat history">
            ...
            <!-- Chat Body -->
            <div class="card-body p-0" x-show="!$wire.isMinimized && !showSettings && !showSessions && !showTemplates && !showSearch">
                ...
                <div class="chat-messages p-3"
                    ...
                    <div class="text-center text-muted py-5">
                        ...
                        <p class="small mb-0">Ask me anything about your farm management, livestock, feeds, or supplies!
                        </p>
                    </div>
                    @else
                    <!-- Messages List -->
                                ...
                                <div class="message-meta mt-2">
                                    ...
                                    <div class="message-actions mt-1">
                                        ...
                                        <div class="small text-muted mb-1">
                                            <span
                                                title="Is Greeting: {{ var_export($message['is_greeting'] ?? false, true) }}">
                                                {{ isset($message['is_greeting']) && $message['is_greeting'] ? '⚡
                                                Greeting' : '🤖 AI' }}
                                            </span>
                                        </div>
                                        @endif
                                        ...
                                        <button class="btn btn-sm btn-outline-primary"
                                            ...
                                            title="Regenerate response" @if(isset($message['id']) &&
                                            isset($ratedMessages[$message['id']])) disabled data-bs-toggle="tooltip"
                                            data-bs-placement="top" data-bs-title="Regeneration disabled after rating"
                                            @endif>
                                    ...
                                    @if(!(isset($message['is_greeting']) && $message['is_greeting'] === true))
                                    <div class="mt-2">
                                        @livewire('chat-rating', ['messageId' => $message['id']],
                ...
                <!-- Message Input -->
                <div class="card-footer bg-white border-top-0 p-3" x-show="!showSettings && !showSessions && !showTemplates && !showSearch">
                    ...
                    <div class="chat-input-container">
                        <form action="#" method="POST" onsubmit="return false;" class="d-flex flex-column gap-2"
                            id="ai-chat-form">
                            <div class="input-group">
                                <textarea wire:model="newMessage" class="form-control chat-textarea" rows="3"
                                    placeholder="Type your message here..."
                                    maxlength="{{ config('chat.security.max_message_length', 5000) }}"
                                    wire:loading.attr="disabled" @disabled($isLoading || !$providerStatus)
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="chat-actions">
                                    <button type="button" class="btn btn-sm btn-outline-secondary me-1"
                                        @click="$wire.set('newMessage', '')" title="Clear message">
