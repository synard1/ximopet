<div id="livewireRecordsContainer">
    ...
    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 mb-6">
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <!-- Yesterday Sales Details -->
            {{-- @if(isset($yesterdayData['sales']) && $yesterdayData['sales']['quantity'] > 0)
            <div class="bg-white rounded-lg p-3 border border-gray-200">
                <h4 class="text-sm font-semibold text-gray-700 mb-2">Detail Penjualan Kemarin</h4>
                <div class="flex justify-between items-center py-1">
                    <span class="text-xs text-gray-600">Jumlah Terjual</span>
                    <span class="text-xs font-medium text-gray-800">{{ $yesterdayData['sales']['quantity'] }}
                        ekor</span>
                </div>
                @if($yesterdayData['sales']['weight'] > 0)
                <div class="flex justify-between items-center py-1">
                    <span class="text-xs text-gray-600">Berat Terjual</span>
                    <span class="text-xs font-medium text-gray-800">{{
                        number_format($yesterdayData['sales']['weight'], 0) }} gr</span>
                </div>
                @endif
                <div class="flex justify-between items-center py-1">
                    <span class="text-xs text-gray-600">Status</span>
                    <span
                        class="text-xs font-medium {{ $yesterdayData['sales']['status'] === 'finalized' ? 'text-green-600' : 'text-orange-600' }}">
                        {{ $yesterdayData['sales']['status'] === 'finalized' ? 'Final' : 'Draft' }}
                    </span>
                </div>
            </div>
            @endif --}}
    ...
    <form wire:submit.prevent="save" class="space-y-8">
        <div class="row g-3">
            ...
            {{-- START: Livestock Summary --}}
            {{-- <div class="col-md-6 d-none d-md-block">
                @isset($livestockSummary)
                <div class="card shadow rounded h-100">
                    <div class="card-header">
                        <strong>Ringkasan Livestock (hingga {{ $livestockSummary['date'] }})</strong>
                    </div>
                    <div class="card-body row text-center">
                        <div class="col-md-4">
                            <div class="fw-bold text-muted">Sisa Ternak</div>
                            <div class="fs-5">{{ number_format($livestockSummary['remaining']) }}</div>
                        </div>
                        <div class="col-md-4">
                            <div class="fw-bold text-muted">Terjual</div>
                            <div class="fs-5">{{ number_format($livestockSummary['sales']) }}</div>
                        </div>
                        <div class="col-md-4">
                            <div class="fw-bold text-muted">Deplesi</div>
                            <div class="fs-5">{{ number_format($livestockSummary['deplesi']) }}</div>
                        </div>
                        <div class="col-md-6">
                            <div class="fw-bold text-muted">Feed Usage</div>
                            <div class="fs-6">{{ number_format($livestockSummary['feed_usage'], 2) }} kg</div>
                        </div>
                        <div class="col-md-6">
                            <div class="fw-bold text-muted">Supply Usage</div>
                            <div class="fs-6">{{ number_format($livestockSummary['supply_usage'], 2) }} unit</div>
                        </div>
                    </div>
                </div>
                @endisset
            </div> --}}
            {{-- END: Livestock Summary --}}
