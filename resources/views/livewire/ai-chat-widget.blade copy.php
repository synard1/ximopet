                    ...
                    <form action="#" method="POST" onsubmit="return false;" class="d-flex flex-column gap-2" id="ai-chat-form">
                        <div class="input-group">
                            <textarea wire:model="newMessage"
                                    ...
                                    @input="console.log('Textarea input event:', $event.target.value, 'Wire value:', $wire.get('newMessage'))"
                                    @focus="console.log('Textarea focused, current value:', $wire.get('newMessage'))"
                                    @blur="console.log('Textarea blurred, current value:', $wire.get('newMessage'))"
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="chat-actions">
                                <button type="button"
                                        ...
                                        @click="console.log('Clear message button clicked, current message:', $wire.get('newMessage')); $wire.call('clearMessage')"
                                        title="Clear message">
...
<script>
  ...
  console.log('Assistant message appended to UI');
    ...
    // Clear Message Debugging
    console.log('AI Chat Widget: Initializing debugging for clear message functionality');

    // Listen for Livewire events
    document.addEventListener('livewire:init', () => {
        console.log('AI Chat Widget: Livewire initialized');
        
        // Listen for message-cleared event
        Livewire.on('message-cleared', (data) => {
            console.log('AI Chat Widget: message-cleared event received:', data);
        });
    });

    // Monitor Livewire property updates
    if (typeof Livewire !== 'undefined') {
        Livewire.hook('morph.updated', ({ el, component }) => {
            if (el.tagName === 'TEXTAREA' && el.hasAttribute('wire:model')) {
                console.log('AI Chat Widget: Textarea updated via Livewire morph:', {
                    element: el,
                    value: el.value,
        ...
        Livewire.hook('request', ({ uri, options, payload, respond, succeed, fail }) => {
            if (payload.calls && payload.calls.some(call => call.method === 'clearMessage')) {
                console.log('AI Chat Widget: clearMessage request detected:', {
                    uri,
                    payload,
    ...
    // Add click event listener to clear message button
    document.addEventListener('click', function(e) {
        if (e.target.closest('button[title="Clear message"]')) {
            console.log('AI Chat Widget: Clear message button clicked directly');
        }
    });
    </script>
    @endpush
